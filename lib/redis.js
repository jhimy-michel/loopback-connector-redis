...
exports.initialize = function initializeSchema(dataSource, callback) {
  ...
  if (dataSource.settings.db) {
    dataSource.settings.options.db = dataSource.settings.db;
  }
  // retry strategy
  dataSource.settings.options.retry_strategy =
    dataSource.settings.retry_strategy;
  ...
  console.log("Settings -> ", dataSource.settings);
  
  var callbackCalled = false;
  var database =
    dataSource.settings.hasOwnProperty("database") &&
...
Client.prototype.multi = function (commands, callback) {
  ...
  this._client.multi(commands).exec(function (err, replies) {
    if (err) console.log(err);
    log();
    if (callback) callback(err, replies);
  });
};
...
BridgeToRedis.prototype.define = function (descr) {
  ...
  this.indexes[m] = {};
  Object.keys(descr.properties).forEach(
    function (prop) {
...
BridgeToRedis.prototype.updateIndexes = function (
  model,
  id,
  data,
  callback,
  ...
  Object.keys(i).forEach(function (key) {
    if (data.hasOwnProperty(key)) {
      var val = data[key];
      schedule.push(["SADD", "i:" + model + ":" + key + ":" + val, id]);
    }
      ...
      if (i) {
        ...
        if (i.name === "Date") {
          if (val.getTime) {
            ...
            typeof val === "number" ||
            (typeof val === "string" && !isNaN(parseInt(val, 10)))
          ) {
  ...
  if (filter.where) {
    ...
    if (filter.where.id) {
      if (filter.where.id.inq) {
        return handleKeys(
          null,
          filter.where.id.inq.map(function (id) {
            return model + ":" + id;
          })
        );
      } else {
...
BridgeToRedis.prototype.destroyAll = function destroyAll(
  model,
  where,
  callback
) {
    ...
    this.all(model, { where: where }, function (err, results) {
      ...
      results.filter(Boolean).forEach(function (result) {
        tasks.push(function (done) {
          br.destroy(model, result.id, done);
        });
      });
...
BridgeToRedis.prototype.updateAttributes = function updateAttrs(
  model,
  id,
  data,
  cb
